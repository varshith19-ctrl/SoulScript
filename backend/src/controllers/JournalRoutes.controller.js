...
export const creatingEntry = async (req, res) => {
  ...
  try {
    ...
    const newEntry = new JournalEntryDB({ text, mood, aiTone ,user:req.userId});
    await newEntry.save();
...
export const readingEntry = async (req, res) => {
  try {
    const entries = await JournalEntryDB.find({ user: req.userId }).sort({ createdAt: -1 });
    res.json(entries);
  } catch (err) {
...
export const loginUser = async (req, res) => {
 ...
 res
      ...
      .json({ message: 'Logged in successfully', user: { username: user.username, email } });  } catch (err) {
